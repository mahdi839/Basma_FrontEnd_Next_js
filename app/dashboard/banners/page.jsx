...
export default function page() {
  ...
  const bannerUrl = process.env.NEXT_PUBLIC_BACKEND_URL + `api/banners?page=${page}`;
  useEffect(() => {
    indexData(bannerUrl)
  }, [page])
            ...
            data.data.map((banner, index) => (
              <tr key={banner.id}>
                ...
                <td className="text-center">  {banner?.banner_images?.map((img) => (
                  <Image className='ml-2 rounded' src={`${process.env.NEXT_PUBLIC_BACKEND_URL}storage/${img.path}`} width={50} height={50} />
                ))}</td>
